...
/usr/local/bin/aws ec2 start-instances --instance-ids ${targets}

echo "# $(date) Waiting for targets to start..."
/usr/local/bin/aws ec2 wait instance-running --instance-ids ${targets}

echo "# $(date) Waiting for target status to be OK..."
/usr/local/bin/aws ec2 wait instance-status-ok --instance-ids ${targets}
